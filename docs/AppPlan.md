...
| Card data hosting | Private GitHub repo — `cards.json` + `version.json` |
| Card data access | Fine-grained read-only PAT injected via `BuildConfig` (from `local.properties`) |
| Local version tracking | Jetpack DataStore (KMP) |
...
enum class Rarity(val id: Int) {
    ...
    LEADER(5), PROMO(6), SP_PROMO(7), SP_CARD(8)
}
```
...
1. **Phase 1** — Gradle scaffold (KMP + Compose Multiplatform + SQLDelight + Koin + Coil + Ktor + DataStore + Tesseract)
2. **Phase 2** — SQLDelight schema (`cards`, `collection`)
3. **Phase 3** — Data layer (models, DB driver factory, repositories, `CardDataUpdater` — fetches JSON from private GitHub repo via PAT, seeds DB, tracks version in DataStore)
...
## Card Data Update Mechanism

Card data is **not bundled** in the app. It lives in a separate private GitHub repository and is fetched at runtime via the GitHub Contents API using a fine-grained read-only PAT.

### Private data repo layout

```
deckamushi-data/           ← private GitHub repo
  version.json             ← {"version": "1.0.0", "updated_at": "2026-04-12"}
  cards.json               ← full card list array
```

### Access

The PAT is stored in `local.properties` (gitignored) and injected at build time:

```
# local.properties  (never committed)
GITHUB_PAT=github_pat_xxxx
GITHUB_DATA_OWNER=your-github-username
GITHUB_DATA_REPO=deckamushi-data
```

```kotlin
// composeApp/build.gradle.kts
buildConfigField("String", "GITHUB_PAT", "\"${localProperties["GITHUB_PAT"]}\"")
buildConfigField("String", "GITHUB_DATA_OWNER", "\"${localProperties["GITHUB_DATA_OWNER"]}\"")
buildConfigField("String", "GITHUB_DATA_REPO", "\"${localProperties["GITHUB_DATA_REPO"]}\"")
```

### Update flow

```
First launch (DB empty)
  └─ Fetch cards.json via GitHub Contents API (Authorization: token {PAT})
  └─ Seed SQLDelight DB
  └─ Persist version string in DataStore

Subsequent launches
  └─ DB is populated → app starts instantly, fully offline

"Update Cards" button
  └─ Fetch version.json → compare with DataStore version
  └─ If newer → fetch cards.json → wipe cards table → re-seed → update DataStore version
  └─ If same → "Already up to date" toast
```

### GitHub Contents API call

```
GET https://api.github.com/repos/{owner}/{repo}/contents/cards.json
Headers:
